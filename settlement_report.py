...
ARCHIVE = Path(os.getenv("AUCTION_ARCHIVE_DIR", f"{_DATA}"))
DAEJEON_CORPS = {
...
# 품목별 표 표시 순서 = 경매 진행 순서 (요청자 5/30 결재 — 경매시간·경매사 순서).
...
AUCTION_BLOCKS = [
    # ── 🥬 채소 파트 ──
    # 요청자 2026-07-18 결재: 송화신 이사 = 시간대(00:00 버섯·엽채 / 01:10 고추·파프리카) 나누지 말고
    #   하나로 합쳐 맨 앞칸에. 라벨을 '송화신 이사'로 통일 → 첫 블록(여기)이라 정렬 순서 0 = 첫칸.
    ("17", None, None, "송화신 이사"),
    # 요청자 2026-07-19: 쌈배추(배추 품종)를 배추에서 분리 → 서병수·김선우 부장(김기영 아님).
    #   _reclassify가 배추 품종 '쌈배추' 레코드의 product를 '쌈배추'로 재라벨 → 이 블록이 잡음.
    ("10", frozenset({"쌈배추"}), None, "00:20 (서병수, 김선우) 부장"),
    # 요청자 2026-07-27: 얼갈이배추·열무의 **4kg 박스**만 서병수·김선우 (2kg·8kg는 김언중 유지).
    ...
    # 요청자 2026-07-20: 쪽파 품종 '깐쪽파' 분리 → 서병수·김선우 (쪽파(일반)은 김언중 유지). _reclassify가 product '깐쪽파'로 재라벨.
    ("12", frozenset({"깐쪽파"}), None, "00:20 (서병수, 김선우) 부장"),
    # 요청자 2026-07-23: 마늘 잎·쫑류(마늘쫑·수입마늘쫑·풋마늘·잎마늘) → 서병수·김선우 (엽채류처럼).
    #   구근 마늘(깐마늘·햇마늘 등)은 오준서 유지. _reclassify가 품종으로 별도 product 재라벨 → 이 블록이 잡음.
    ("12", frozenset({"마늘쫑", "수입마늘쫑", "풋마늘", "잎마늘"}), None, "00:20 (서병수, 김선우) 부장"),
    # 요청자 2026-07-23: 여주(부류21) → 서병수·김선우.
    ("21", frozenset({"여주"}), None, "00:20 (서병수, 김선우) 부장"),
    # 요청자 2026-07-23: 유채(부류16)·참당귀(부류19) → 서병수·김선우 (공식 정답지 대조, 나머지에서 이동).
    ("16", frozenset({"유채"}), None, "00:20 (서병수, 김선우) 부장"),
    ("19", frozenset({"참당귀"}), None, "00:20 (서병수, 김선우) 부장"),
    ("13", None, frozenset({"파프리카", "피망(단고추)"}), "00:20 (서병수, 김선우) 부장"),
    ("14", None, frozenset({"도라지", "더덕", "마", "무청", "시레기"}), "00:20 (서병수, 김선우) 부장"),  # 도라지·더덕·마→오준서, 무청·시레기→김기영 제외(2026-07-20)
    ("03", None, None, "00:20 (서병수, 김선우) 부장"),
    ("09", None, None, "00:30 강신창 부장"),
    ("12", frozenset({"꽈리고추", "풋고추", "홍고추", "건고추"}), None, "송화신 이사"),  # 건고추=공식 송화신(2026-07-20)
    ("13", frozenset({"파프리카", "피망(단고추)"}), None, "송화신 이사"),
    # 요청자 2026-07-06 결재: 김기영·김언중 공동 → 품목별 분리 (앞으로 계속 적용)
    #   김기영 = 배추·무·대파·양배추 / 김언중 = 열무·쪽파·옥수수·알타리·얼갈이·갓·실파
    ("10", frozenset({"배추", "양배추"}), None, "김기영 부장"),
    ("14", frozenset({"무청", "시레기"}), None, "김기영 부장"),  # 무청·시레기=공식 김기영(2026-07-20)
    ...
    ("04", frozenset({"옥수수", "기장"}), None, "김언중 부장"),  # 기장=공식 김언중(2026-07-20)
    ("05", None, None, "이용수 부장"),
    ("11", frozenset({"당근"}), None, "이용수 부장"),
    ("12", frozenset({"양파"}), None, "이용수 부장"),
    ("10", frozenset({"숙주나물", "우엉대", "콩나물"}), None, "오준서 경매사"),  # 토란대 → 서병수·김선우(2026-07-20)
    ("11", frozenset({"연근", "우엉", "토란"}), None, "오준서 경매사"),
    ("12", frozenset({"마늘", "생강"}), None, "오준서 경매사"),  # 건고추 → 송화신(2026-07-20)
    # 요청자 2026-07-20: 부류14 산채류 중 도라지·더덕·마만 오준서로 (나머지 산채류=새싹·참나물·취나물 등은 서병수·김선우 유지).
    ("14", frozenset({"도라지", "더덕", "마"}), None, "오준서 경매사"),
    ("91", None, None, "오준서 경매사"),
    # ── 🍎 과일 파트 ──
    ("06", frozenset({"매실", "복숭아", "보리수", "블루베리", "살구", "앵두", "오디", "자두"}), None, "04:30 이기송 부장"),
    ("06", frozenset({"패션푸룻"}), None, "04:30 이기송 부장"),  # 요청자 2026-07-23: 패션후르츠 → 이기송
    ("06", frozenset({"석류"}), None, "04:30 이기송 부장"),  # 요청자 2026-07-23: 국산 석류 → 이기송 (수입석류는 안대명, _reclassify 분리)
    ("07", frozenset({"은행"}), None, "04:30 이기송 부장"),  # 요청자 2026-07-23: 은행 → 이기송
    ("18", frozenset({"수삼"}), None, "04:30 이기송 부장"),  # 요청자 2026-07-23: 수삼 → 이기송
    ("07", frozenset({"대추", "밤", "잣", "호두"}), None, "04:30 이기송 부장"),  # 국산 호두=공식 이기송 (수입호두는 안대명 유지, 2026-07-20)
    ("08", frozenset({"딸기", "멜론", "방울토마토", "토마토"}), None, "04:30 이기송 부장"),
    ("06", frozenset({"곶감", "단감", "떫은감", "포도"}), None, "(김상걸, 차수호) 이사"),
    ("08", frozenset({"참외"}), None, "(김상걸, 차수호) 이사"),
    ("06", frozenset({"감귤", "만감", "탄제린", "금감"}), None, "윤정기 이사"),  # 탄제린=공식 윤정기(2026-07-20) / 금감=공식 윤정기(2026-07-23)
    ("08", frozenset({"수박"}), None, "윤정기 이사"),
    ("06", frozenset({"배", "사과"}), None, "이광진 부장"),
    ("06", frozenset({"듀리안", "레몬", "망고", "망고스턴", "바나나", "아로니아",
                      "아보카도", "오렌지", "용과", "자몽", "참다래(키위)", "체리",
                      "코코넛", "파인애플"}), None, "(안대명, 심세영) 부장 (수입과일)"),
    ("07", frozenset({"다래"}), None, "(안대명, 심세영) 부장 (수입과일)"),
    # 요청자 2026-07-20: 수입 곶감·호두 → 안대명·심세영 (수입 품종/원산지 분리). 국산 곶감=김상걸·차수호, 국산 호두=이기송(위 07 블록). _reclassify가 '수입곶감'·'수입호두'로 재라벨.
    ("06", frozenset({"수입곶감", "수입석류"}), None, "(안대명, 심세영) 부장 (수입과일)"),  # 수입석류=요청자 2026-07-23
    ("07", frozenset({"수입호두"}), None, "(안대명, 심세영) 부장 (수입과일)"),
    # 요청자 2026-07-21: 수입 포도·블루베리·멜론 → 안대명·심세영. 국산은 원래대로
    #   (포도=김상걸·차수호 / 블루베리·멜론=이기송). _reclassify가 '수입포도'·'수입블루베리'·'수입멜론'으로 재라벨.
    ("06", frozenset({"수입포도", "수입블루베리"}), None, "(안대명, 심세영) 부장 (수입과일)"),
    ("08", frozenset({"수입멜론"}), None, "(안대명, 심세영) 부장 (수입과일)"),
    # 요청자 2026-07-18 결재: 수입 땅콩 → 안대명·심세영 (수입과일과 같은 경매사).
    #   대전 4법인 수입 땅콩은 전량 중국산·품종 '땅콩(수입)'으로 라벨 일치 → 국산 땅콩과 안전 분리.
    #   판정은 load_day에서 수입 레코드의 product를 '수입땅콩'으로 재라벨(아래 _reclassify_import).
    ("16", frozenset({"수입땅콩"}), None, "(안대명, 심세영) 부장 (수입과일)"),
    # 요청자 2026-07-18 결재: 국산 땅콩 → 이기송 부장 (앞으로 계속). 수입은 위 안대명·심세영.
...
# (요청자 5/30 방식2 결재: 한 경매사가 여러 부류를 다뤄도 부류 무시하고 그 경매사 품목 전체를 금액순으로)
_LABEL_ORDER = {}
...
def _reclassify(item: dict) -> dict:
    ...
    - 쪽파: 품종 '깐쪽파' → '깐쪽파'(서병수·김선우) / 쪽파(일반)은 '쪽파' 유지(김언중).
    - 곶감·호두: 수입(품종 '수입' 또는 원산지 외국) → '수입곶감'·'수입호두'(안대명·심세영) / 국산은 유지.
    - 포도·블루베리·멜론: 수입 → '수입포도'·'수입블루베리'·'수입멜론'(안대명·심세영) / 국산은 원래대로
      (포도=김상걸·차수호 / 블루베리·멜론=이기송). 대전 노은 수입 버킷은 전량 품종 '(수입)'+원산지 외국
      두 신호 일치 → 국산(샤인마스캇·거봉 등) 오분류 0 (2026-07-21 상반기 스캔 확인).
    요청자 2026-07-18(땅콩)·07-19(쌈배추)·07-20(깐쪽파·수입곶감·수입호두)·07-21(수입포도·블루베리·멜론)."""
    product = item.get("product") or ""
    variety = item.get("variety") or ""
    origin = item.get("origin") or ""
    imported = "수입" in variety or any(k in origin for k in _IMPORT_ORIGIN_KEYS)
    if product == "땅콩":
        if imported:
        ...
        # 요청자 2026-07-27: 얼갈이배추·열무의 **4kg 박스만** 서병수·김선우 (2kg·8kg 등은 김언중 유지).
        ...
        if imported:
            return {**item, "product": "수입호두"}
    elif product == "포도":
        if imported:
            return {**item, "product": "수입포도"}
    elif product == "블루베리":
        if imported:
            return {**item, "product": "수입블루베리"}
    elif product == "멜론":
        if imported:
            return {**item, "product": "수입멜론"}
    elif product == "석류":
        # 요청자 2026-07-23: 수입석류 → 안대명·심세영(수입과일) / 국산석류 → 이기송. (대전 현재 전량 수입)
        if imported:
            return {**item, "product": "수입석류"}
    elif product == "마늘":
        # 요청자 2026-07-23: 잎·쫑류(마늘쫑·풋마늘·잎마늘)는 엽채류처럼 서병수·김선우 → 별도 product로 분리.
        #   구근류(깐마늘·햇마늘·마늘(수입))는 '마늘' 유지(오준서). 품종명으로 정확 판별(구근은 안 걸림).
        if variety.startswith("마늘쫑"):
            return {**item, "product": "수입마늘쫑" if imported else "마늘쫑"}
        if variety.startswith("풋마늘"):
            return {**item, "product": "풋마늘"}
        if variety.startswith("잎마늘"):
            return {**item, "product": "잎마늘"}
    return item
...
# 요청자 결재(2026-05-29): 3일 → 갱신(2026-07-07): 4일.
#   근거: 모든 법인은 정산자료를 D+4는 지나야 완비된다(요청자 도메인 재확인). find_last_settled_day는
...
def aggregate_by_product(records):
    ...
    # (2026-06-01 요청자 결재: 표의 표시 금액=중앙청과 기준과 정렬을 일치시킴.
...
def product_table(sorted_products, product_corp, product_cat):
    ...
    (요청자 5/30: 금액·물량 순서 + 경매사 블록 내부 금액순 / 6/19: 경매사 블록 소계 행)
    (요청자 7/1: 품목 16개 이상 경매사 블록은 물량 많은 순 상위 15개만 표시 + 나머지 '기타'로 합산.
...
{product_table(*product_data)}"""
        ...
        subtitle = (f"기간: {period} (정산 완료일 기준) | 영업 {days}일 | 생성: {now}<br>"
                    "출처: 농산물유통정보(aT) 정산정보 API | 4법인: 대전중앙청과·원협노은·대전청과·농협대전")
        # 공판장 5/1~5/8 정산 자료 중복 이관 오류 주의 (요청자 도메인 확인, 2026-06-04).
