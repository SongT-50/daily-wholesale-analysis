...
AUCTION_BLOCKS = [
    ...
    # 태은이 2026-07-20: 쪽파 품종 '깐쪽파' 분리 → 서병수·김선우 (쪽파(일반)은 김언중 유지). _reclassify가 product '깐쪽파'로 재라벨.
    ("12", frozenset({"깐쪽파"}), None, "00:20 (서병수, 김선우) 부장"),
    # 태은이 2026-07-23: 마늘 잎·쫑류(마늘쫑·수입마늘쫑·풋마늘·잎마늘) → 서병수·김선우 (엽채류처럼).
    #   구근 마늘(깐마늘·햇마늘 등)은 오준서 유지. _reclassify가 품종으로 별도 product 재라벨 → 이 블록이 잡음.
    ("12", frozenset({"마늘쫑", "수입마늘쫑", "풋마늘", "잎마늘"}), None, "00:20 (서병수, 김선우) 부장"),
    # 태은이 2026-07-23: 여주(부류21) → 서병수·김선우.
    ("21", frozenset({"여주"}), None, "00:20 (서병수, 김선우) 부장"),
    # 태은이 2026-07-23: 유채(부류16)·참당귀(부류19) → 서병수·김선우 (공식 정답지 대조, 나머지에서 이동).
    ...
    # 태은이 2026-07-20: 부류14 산채류 중 도라지·더덕·마만 오준서로 (나머지 산채류=새싹·참나물·취나물 등은 서병수·김선우 유지).
    ("14", frozenset({"도라지", "더덕", "마"}), None, "오준서 경매사"),
    ("91", None, None, "오준서 경매사"),
    # ── 🍎 과일 파트 ──
    ("06", frozenset({"매실", "복숭아", "보리수", "블루베리", "살구", "앵두", "오디", "자두"}), None, "04:30 이기송 부장"),
    ("06", frozenset({"패션푸룻"}), None, "04:30 이기송 부장"),  # 태은이 2026-07-23: 패션후르츠 → 이기송
    ("06", frozenset({"석류"}), None, "04:30 이기송 부장"),  # 태은이 2026-07-23: 국산 석류 → 이기송 (수입석류는 안대명, _reclassify 분리)
    ("07", frozenset({"은행"}), None, "04:30 이기송 부장"),  # 태은이 2026-07-23: 은행 → 이기송
    ("18", frozenset({"수삼"}), None, "04:30 이기송 부장"),  # 태은이 2026-07-23: 수삼 → 이기송
    ...
    # 태은이 2026-07-20: 수입 곶감·호두 → 안대명·심세영 (수입 품종/원산지 분리). 국산 곶감=김상걸·차수호, 국산 호두=이기송(위 07 블록). _reclassify가 '수입곶감'·'수입호두'로 재라벨.
    ("06", frozenset({"수입곶감", "수입석류"}), None, "(안대명, 심세영) 부장 (수입과일)"),  # 수입석류=태은이 2026-07-23
    ("07", frozenset({"수입호두"}), None, "(안대명, 심세영) 부장 (수입과일)"),
    # 태은이 2026-07-21: 수입 포도·블루베리·멜론 → 안대명·심세영. 국산은 원래대로
...
def _reclassify(item: dict) -> dict:
    ...
    태은이 2026-07-18(땅콩)·07-19(쌈배추)·07-20(깐쪽파·수입곶감·수입호두)·07-21(수입포도·블루베리·멜론)."""
    ...
    if product == "땅콩":
        ...
        # 태은이 2026-07-23: 수입석류 → 안대명·심세영(수입과일) / 국산석류 → 이기송. (대전 현재 전량 수입)
        if imported:
            return {**item, "product": "수입석류"}
    elif product == "마늘":
        # 태은이 2026-07-23: 잎·쫑류(마늘쫑·풋마늘·잎마늘)는 엽채류처럼 서병수·김선우 → 별도 product로 분리.
